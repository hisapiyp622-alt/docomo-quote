...
(function () {
  ...
  function renderOpts() {
    ...
    IENAKA_OPTS.forEach(function (o) {
      ...
      h += '<label class="check ienaka-opt' + (o.needsPhone ? " sub" : "") + '"><input type="checkbox" data-ieopt="' + o.id + '"' + (state.opts[o.id] ? " checked" : "") + "> "
        + esc(o.name) + ' <span class="opt-price"><input type="number" data-ieoptprice="' + o.id + '" value="' + pr + '" style="width:5.5em;text-align:right;padding:4px 6px;border:1px solid var(--line);border-radius:5px;font:inherit">円/月</span>'
  ...
  function bind() {
    ...
    $("ieProvider").addEventListener("change", function () { state.provider = this.value; syncForm(); recalc(); });
    ...
    $("ieRouter10gPrice").addEventListener("input", function () { state.router10gPrice = num(this.value); recalc(); });
