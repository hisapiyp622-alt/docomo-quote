...
(function () {
  ...
  function flowData(r) {
    ...
    if (state.product === "home5g") {
      ...
      /* 事業者変更は「事業者変更承諾番号」が要る。取り方が事業者で違うため、
       * 店頭の運用に合わせて工程を分ける（店舗の指定・2026-08-09）。 */
      var jgLine = curLineDef();
      var jg = state.applyType === "jigyosha" ? ((jgLine && jgLine.id) || "") : "";
      if (jg === "ocn") {
        // OCNはその場でお電話して承諾番号を受け取ってからお申込み
        step("事業者変更承諾番号の取得（OCNへお電話）",
          "店頭でOCNへお電話し、事業者変更承諾番号をお受け取りいただきます", "phone", "本日・店頭");
        noteTo("承諾番号", "OCNのご連絡先: 0120-506-506（OCNカスタマーズフロント）");
        noteTo("承諾番号", "**日曜・祝日はお電話がつながりません。**その場合は後日、お客様からお電話いただき、承諾番号を店舗へお知らせください");
      }
      step("お申込み", "本日、店頭でお手続きが完了しました", "shop", "本日");
      if (jg === "biglobe") {
        // BIGLOBEは承諾番号を後日受け取る。受け取ったらドコモ光へ伝えて切替日が決まる
        step("事業者変更承諾番号のお手続き（BIGLOBEへお電話）",
          "店頭でお時間があれば、承諾番号を受け取るためのご予約をこの場でお取りします", "phone", "本日・店頭");
        noteTo("お手続き", "BIGLOBEのご連絡先: 0120-86-0962（ビッグローブ カスタマーサポート）");
