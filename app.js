...
(function () {
  ...
  // 社内版は端末内ロックを使わない（製品版のみ）
  function lockEnabled() { return false; }
  // マスタ設定専用のパスワードを決めているか
  // 社内版はマスタ設定パスワードを使わない（製品版のみ）
  function adminLockEnabled() { return false; }
  ...
  var HIST_KEY = "dq-master-hist-v1";
  var HIST_MAX = 20;
  ...
  /* 社内版はログインしないので、店舗で1つのドキュメントを共有する。
   * 同期を切りたい端末のために、ヘッダの担当者セレクタで「同期OFF」を選べる。 */
  var SYNC_ON_KEY = "dq-sync-on";
  function syncOnSaved() {
    try { return localStorage.getItem(SYNC_ON_KEY) !== "0"; } catch (e) { return true; }
  }
  function cloudOn() { return CLOUD.enabled && CLOUD.syncOn && CLOUD.db; }
  ...
  function storeDoc() { return CLOUD.db.collection("settings").doc("docomoQuoteStore"); }
  ...
  // 社内版は担当者コードで入り直さない（ヘッダの担当者セレクタで切り替える）
  function anyStaffCode() { return false; }
  ...
  /* ヘッダの担当者セレクタ。
   * 同じ担当を選んだ端末どうしで、見積もり・保存・テンプレートが同期される。 */
  function renderStaffSelect() {
    var sel = $("staffSelect");
    if (!sel) return;
    sel.innerHTML = config.staff.map(function (s) {
      return '<option value="' + esc(s.id) + '">' + esc(s.name || "担当") + "</option>";
    }).join("") + '<option value="__off">同期OFF</option>';
    sel.value = CLOUD.syncOn ? (config.activeStaffId || config.staff[0].id) : "__off";
  }
  /* 同期の入切は監視の張り直しが多いため、読み直して確実に切り替える */
  function setSyncOn(on) {
    try { localStorage.setItem(SYNC_ON_KEY, on ? "1" : "0"); } catch (e) {}
    location.reload();
  }
  function initStaffSelect() {
    ...
    sel.addEventListener("change", function () {
      if (this.value === "__off") { setSyncOn(false); return; }
      if (!CLOUD.syncOn) { setSyncOn(true); return; }
      var id = this.value;
  ...
  var STORE_UID_KEY = "dq-store-uid";
  ...
  // 社内版は自動ログアウトしない
  function armIdle() { return; }
  function armIdleUnused(on) {
    IDLE.armed = !!on;
    IDLE.last = Date.now();
  ...
  function initCloud() {
    CLOUD.syncOn = syncOnSaved();
    // 店舗ログイン・マスタ設定パスワードの欄は社内版では使わない
    var lb0 = $("lockBox"); if (lb0) lb0.hidden = true;
    var ab0 = $("adminLockBox"); if (ab0) ab0.hidden = true;
    var configured = typeof firebase !== "undefined" && firebase.apps && firebase.apps.length;
    if (!configured) { syncStatus("", ""); afterStoreLogin(); return; }
    try { CLOUD.db = firebase.firestore(); } catch (e) { syncStatus("", ""); afterStoreLogin(); return; }
    CLOUD.enabled = true;
    if (!cloudOn()) { syncStatus("同期OFF", ""); afterStoreLogin(); return; }
    syncStatus("同期中…", "");
    watchStore();
    storeDoc().get().then(function (snap) {
      if (snap.exists) applyRemoteStore(snap.data());
      afterStoreLogin();
    }, function () {
  ...
  // 社内版はマスタ設定をそのまま開ける
  function masterGateOn() { return false; }
