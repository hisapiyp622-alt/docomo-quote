...
(function () {
  ...
  /* 営業用デモ（/keitai-demo/）から読み込まれたときの印。
   * デモはログイン無し・クラウド無し・この端末の中だけ。同じ住所に置く製品版と
   * 保存領域がぶつからないよう、接頭辞を分ける（kqdemo）。
   * 生成は tools/build-keitai-demo.js。 */
  var DEMO = typeof window !== "undefined" && !!window.KEITAI_DEMO;
  ...
  function loadConfig() {
    ...
    /* デモ: はじめて開いたときに店名と担当者を入れておく。
     * 初期設定の画面を出さず、いきなり見積もりの画面から始めるため。 */
    if (DEMO && !String(config.storeName || "").trim()) {
      config.storeName = "デモ店舗";
      config.storeTel = "";
      if (config.staff && config.staff.length === 1 && !String(config.staff[0].name || "").trim()
        || (config.staff && config.staff.length === 1 && config.staff[0].name === "担当1")) {
        config.staff[0].name = "デモ担当";
      }
    }
  ...
  function wizNeeded() {
    if (DEMO) return false;   // デモは店名・担当者を入れてあるので初期設定を出さない
  ...
  showMovedOutNote();
  /* デモ（/keitai-demo/）: 何が違うのかを画面の上に出しておく。
   * 「本物と間違えて実務に使う」「入力が同期されると思って待つ」を防ぐため。 */
  if (DEMO) {
    (function () {
      var el = document.getElementById("cloudWarn");
      if (!el) return;
      el.innerHTML = "これは<b>デモ版</b>です。入力はこの端末の中だけに保存され、"
        + "ほかの端末とは同期しません。通信が切れると使えません。";
      el.hidden = false;
    })();
  }
  if (devHost()) {
