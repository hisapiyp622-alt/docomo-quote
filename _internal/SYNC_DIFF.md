...
ルールは **2026-08-16 に修正済み**です。それ以前は `settings/` の**サブコレクションに効いておらず**、
見積もり本体は届くのに `quotes/` `saved/` `templates/` だけが同期されない状態でした。

ヘッダーの表示が「同期:権限エラー」になるときは、recipe-box の Firestore ルールで
`settings/` 配下が許可されていません。Firebaseコンソール → Firestore Database → ルールが
次の形になっているか確認してください。**サブコレクションまで含めることが要点です**
（社内版は認証を使わないため、この2つのドキュメントだけを開けます）。
