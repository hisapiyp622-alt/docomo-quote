...
| アプリ | パス | 版 | キャッシュ |
|---|---|---|---|
| 製品版ケータイ（フロントーク） | `keitai-app/` | **1.108.0** | kq-v213 |
...
**別紙①②とも作成済み・弁護士レビューは行わない方針（2026-08-16）。残るのは締結だけ**:
- 秘密保持3年・自動更新の申し出2か月前・是正30日
- 月額: 管理費10%控除後の残りを50/50・翌月末払い
- 初期費用: **甲（安藤さん）へ1店舗あたり30,000円の固定額**（乙が値付けを変えても不変）
- 残: **締結のみ**（別紙②サポート分担表は 2026-08-16 に作成済み。弁護士レビューは行わない）
- **締結日と甲住所はファイルに書かない**（リポジトリが公開のため。締結時の紙に直接記入）

### ③ 器の移管（販売開始前が期限）
- [x] 独自ドメイン … `frontalk.curacon.co.jp`（2026-08-18 公開済み）
- [ ] Firebase（`keitai-quote`）に Curacon の Google アカウントを**オーナー追加**
      … アカウントは 2026-08-19 に受領済み。**残るはコンソール操作だけ**（このセッションに
      Google Cloud の認証情報が無いため代行できない）。
      追加後は Curacon 側も店舗IDの発行・停止ができるようになる＝安藤さん1人に依存しない形になる
- [ ] GitHub の権限付与（`docomo-quote`・`frontalk`）… アカウント名は未受領
