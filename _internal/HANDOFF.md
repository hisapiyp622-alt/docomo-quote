...
| アプリ | パス | 版 | キャッシュ |
|---|---|---|---|
| 製品版ケータイ（ミツモリン） | `keitai-app/` | **1.104.0** | kq-v207 |
| 社内版ケータイ | `/`（生成物） | 同上 | dq-v207 |
...
### ① Firestore ルールの修正（ユーザー作業・最優先）
社内版の同期不具合の**根本原因**。recipe-box プロジェクトの現在のルールは
`match /settings/{document}` で、**サブコレクションに効いていない**。
そのため `quotes/` `saved/` `templates/` が同期されない。次に直してもらう:
...
**ユーザーが実施したか未確認。次のセッションで確認すること。**
直ったかの確認方法: 2台で同じ担当を開き、片方で入力 → もう片方に反映されるか。
